<div class="card card-info mt-4" id="param" style="display: none;">
    <div class="card card-secondary mt-4">
        <div class="card-header">
            <h3 class="card-title">Parametros de Facturacion</h3>
        </div>
            <div class="card card-body">
                <ul class="nav nav-tabs" id="recyseParamsTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" id="tab-divisor" data-toggle="tab" href="#content-divisor" role="tab" aria-controls="content-divisor" aria-selected="true">
                            <i class="fas fa-divide mr-2"></i>Division de KG
                        </a>
                    </li>
                </ul>
                    <div class="tab-content" id="recyseParamsTabContent">
                        <div class="tab-pane fade show active" id="content-divisor" role="tabpanel" aria-labelledby="tab-divisor">
                            <div class="mt-3">
                                <button class="btn btn-warning btn-sm mb-3" data-toggle="modal" data-target="#modalNuevoDivisor">
                                    <i class="fas fa-plus"></i> Cambiar factor divisor
                                </button>
                ...
                <div class="mt-3">
                    ...
                    <div class="card card-outline card-success">
                        ...
                        <div class="card-body p-0">
                            ...
                            <input type="text" id="searchCategorias" class="form-control" placeholder="Buscar categoría...">
...
<!-- Modal para Nuevo Divisor -->
<div class="modal fade" id="modalNuevoDivisor" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title">Cambiar factor divisor</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <form id="formDiv">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="valor">Nuevo factor divisor</label>
                        <input type="number" class="form-control" id="valorDiv" placeholder="Cantidad de kg por un punto, Ej: 10kg = 1pto" name="valor" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                    <button type="submit" class="btn btn-success">Actualizar factor divisor</button>
                </div>
                <div id="alertContainer_div"></div>
            </form>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function(){
        ...
        // ELIMINÉ LA BÚSQUEDA POR AJAX PARA PRODUCTOS
                    ...
                    data.categorias.forEach((categoria, index) => {
                        ...
                        fila.innerHTML = `
                            <td class="text-center" style="border-left: 3px solid #17a2b8;">
                                <span class="badge bg-info">${id++ || (index + 1)}</span>
                            </td>
                            <td>
    ...
    // Función para cargar tabla de productos (SIN parámetros de búsqueda)
    async function cargarTablaProductos() {
        try {
            const response = await fetch('./php/productosParam.php');
            const data = await response.json();
    ...
    function AlertaDivisor(mensaje, tipo = 'danger', tiempo = 5000){
        const alertContainer = document.getElementById('alertContainer_div');
    ...
    function alertaDivisorpo(mensaje, tipo = 'success', tiempo = 5000){
        const alertContainer = document.getElementById('alertContainer_div');
    ...
    document.addEventListener('DOMContentLoaded', function(){
        const form_recyse = document.getElementById('formDiv');

        form_recyse.addEventListener('submit', function(evento){
            evento.preventDefault();
            
            const nuevoDiv = new FormData(form_recyse);
            ...
            xhr.onreadystatechange = function(){
                ...
                if (respuesta.status === "error") {
                  ...
                  alertaDivisorpo(respuesta.message);
                  form_reg.reset();
                } else if (respuesta.status === "alert"){
                  AlertaDivisor(respuesta.alerta);
                }
            };

            xhr.open('POST', './php/paramRecyse.php', true);
            xhr.send(nuevoDiv);
        });
    });
...
<script>
    ...
    $(document).ready(function() {
        // Buscador de Categorías - IDÉNTICO A TU OTRO CÓDIGO
        $('#searchCategorias').on('keyup', function() {
            var value = $(this).val().toLowerCase();
            $('#tabla-categorias tr').filter(function() {
                $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
            });
        });

        // Buscador de Productos - IDÉNTICO A TU OTRO CÓDIGO
        ...
        $('#filtroCategoria').on('change', function() {
            var categoriaSeleccionada = $(this).find('option:selected').text();
            
            if (categoriaSeleccionada === "Todas las categorías") {
                // Mostrar todos
                $('#tabla-productos tr').show();
            } else {
                // Filtrar por categoría
                $('#tabla-productos tr').each(function() {
                    var catCelda = $(this).find('td:nth-child(3)').text().trim();
                    $(this).toggle(catCelda === categoriaSeleccionada);
                });
            }
        });
    });
