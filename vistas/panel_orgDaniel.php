<div class="car card-success" id="panel_org" style="display: none">
    ...
    <table class="table table-striped table-bordered table-secondary" style="border: 2px solid;">
        <thead style="padding: 2px">
            ...
            <th colspan="2" style="text-align: center;"><button class="btn btn-secondary btn-sm" onclick="mostrar_formOrg()">Nueva organización</button></th>
        </thead>
        <tbody>
                ...
                data.recibos.forEach(r => {
                    ...
                    if (r.estado === '0') {
                        ...
                        fila.innerHTML = `
                            ...
                            <td>${r.estado}</td>
                            <td>
                                <button class="btn btn-warning btn-sm btn-detalle" data-id="${r.correlativo}">Detalles de material</button>
                                <a class="btn btn-danger btn-sm" href="./php/pdf_re.php?c=${r.correlativo}" target="_blank">Ver PDF</a>
                                <button class="btn btn-primary btn-sm btn-refinal" data-id="${r.correlativo}" onclick="mostrar_formRefinal()">Crear recibo final</button>
                            </td>
                        `;
                    } else {
  ...
  document.addEventListener('click', function(e){
    if(e.target.classList.contains('btn-refinal')){
      ...
      .then(data_cor => {
        ...
        const tabla_materiales = document.getElementById('cuerpo_materiales_ref');
        tabla_materiales.innerHTML = "";

        if (data_cor.materiales_ref.length > 0) {
          data_cor.materiales_ref.forEach(m => {
            const fila_m = document.createElement('tr');
            fila_m.innerHTML = `
              <td>${m.descripcion}</td>
              <td><input type="number" value="0" class="form-control cantidad" name="${m.id}_cantidad"></td>
        ...
        activarCalculoMateriales1();


            function activarCalculoMateriales1() {
              const filas = document.querySelectorAll('#cuerpo_materiales_ref tr');
              ...
              filas.forEach(fila => {
                ...
                if (inputCantidad && inputPuntaje) {
                  inputCantidad.addEventListener('input', function () {
                    const cantidad = parseFloat(this.value) || 0;
                    const divisor = parseFloat(data_cor.division.division_kilo);
                    const puntaje = cantidad / divisor;
                    inputPuntaje.value = puntaje.toFixed(2);
...
<script>
      ...
      function mostrarAlerta_pre(mensaje, tipo = 'danger', tiempo = 5000){
    ...
    function alertaPositiva_pre(mensaje, tipo = 'success', tiempo = 5000){
      ...
      function mostrarAlerta_re(mensaje, tipo = 'danger', tiempo = 5000){
