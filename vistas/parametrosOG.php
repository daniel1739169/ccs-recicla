    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="formDiv">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="valorDivisor">Nuevo factor divisor</label>
                        <input type="number" class="form-control" id="valorDivisor" placeholder="Cantidad de kg por un punto, Ej: 10kg = 1pto" name="valor" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
...
<script>
                    ...
                    data.categorias.forEach((categoria, index) => {
                        ...
                        fila.innerHTML = `
                            <td class="text-center" style="border-left: 3px solid #17a2b8;">
                                <span class="badge bg-info">${categoria.id || (index + 1)}</span>
                            </td>
                            <td>
        ...
        try {
            ...
            document.addEventListener('DOMContentLoaded', function(){
                ...
                xhr.onreadystatechange = function(){
                    if (xhr.readyState === 4) {
                        if (xhr.status === 200) {
                            alertaDivisorpo(xhr.responseText);
                            form_recyse.reset();
                            
                        }else{
                            alertaDivisor('Envio no realizado');
                        }
                    }
                };
  ...
  function filtrarProductos() {
    var texto = $('#buscarProducto').val().toLowerCase();
    var categoria = $('#filtroCategoria').val();

    $('#tabla-Productos tr').filter(function() {
      var fila = $(this).text().toLowerCase().indexOf(texto) > -1;
      var coincideCategoria = true;

      if (categoria) {
        // Busca la celda de categoría (3ra columna)
        var catCelda = $(this).find('td:nth-child(3)').text().trim();
        coincideCategoria = (catCelda === categoria);
      }

      $(this).toggle(fila && coincideCategoria);
    });
  }
});
